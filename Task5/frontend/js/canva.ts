...
class Excel {
   ...
   */
  constructor(container: HTMLElement, fileId: string = "-1") {
   ...
   */
  async extendCells(count: number, axis: "X" | "Y") {
    if (!this.canvas.data.length) {
  ...
  getExcelAPIUrls(host: string) {
    return {
      createOrUpdateCell: (data: Cell) =>
        fetch(host + data.id, {
          method: "PUT",
          body: JSON.stringify({
            ...data,
